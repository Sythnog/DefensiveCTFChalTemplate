...
function StartJudge() {
    ...
    httpReq.onreadystatechange = function() {
        if (httpReq.readyState == XMLHttpRequest.DONE) {
            ...
            if (respJson["failed"] == true) {
                ...
                window.location = `http://${window.location.hostname}:8080/failed`;
            } else {
                // Send the user to the done page, where they will get the result from the validation/attack
                window.location = `http://${window.location.hostname}:8080/done`;
            }
            console.log(options)
        }
    }
    ...
    httpReq.open("post", `http://${window.location.hostname}:8080/start`, true);
    httpReq.send(); 
}
